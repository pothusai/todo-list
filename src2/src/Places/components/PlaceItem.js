...
const PlaceItem = props => {
    ...
    return (
        ...
        <Modal show={showConfirmModal} onCancel={cancelDeleteHandler} header="Are you sure" footerClass="place-item__model-actions" footer={
            <React.Fragment>
        ...
        <Card className="place-item__content">
            ...
            <div className="place-item__actions">
                <Button inverse onClick={openMapHandler}>VEIW ON MAP</Button>
                {auth.isLoggedIn && (
                    <Button to={`/places/${props.id}`}>EDIT</Button>
                )}
                {auth.isLoggedIn && (
                    <Button dnager onClick={showDeleteWarningHandler}>DELETE</Button>
                )}
            </div>
        </Card>
    </li>
